...
char *tracer_pack_pkt(brdcast_hdr * bcast_hdr, tracer_hdr * trcr_hdr,
					  tracer_chunk * tracer, char *bblocks,
					  size_t bblocks_sz, int new_bblocks);
 ...
 */
int
ip_to_rfrom(inet_prefix rip, quadro_group * rip_quadg,
 ...
 */
int
tracer_verify_pkt(tracer_chunk * tracer, u_short hops, inet_prefix rip,
 ...
 */
int
tracer_add_rtt(int rpos, tracer_chunk * tracer, u_short hop)
{
 ...
 */
int
tracer_get_trtt(int from_rnode_pos, tracer_hdr * trcr_hdr,
 ...
 */
void
tracer_update_gcount(tracer_hdr * trcr_hdr, tracer_chunk * tracer,
 ...
 */
bnode_hdr *
tracer_build_bentry(void *void_map, void *void_node,
					...
					int *bnode_links, u_char level)
{
	...
	bchunk =
		(bnode_chunk *) (bnode_gid + sizeof(u_char) * bhdr->bnode_levels);
	...
	if (bhdr->links < me.bnode_map[level][bm].links) {
		...
		bhdr = (bnode_hdr *) bblock;
		bchunk =
			(bnode_chunk *) (bblock + BNODE_HDR_SZ(bhdr->bnode_levels));
	}
 ...
 */
int
tracer_pkt_build(u_char rq, int rq_id, int bcast_sub_id,
				 int gnode_id, u_char gnode_level,
				 brdcast_hdr * bcast_hdr, tracer_hdr * trcr_hdr,
				 tracer_chunk * tracer, u_short old_bblocks,
				 char *old_bblock_buf, size_t old_bblock_sz, PACKET * pkt)
{
	brdcast_hdr bh;
	tracer_hdr th;
	...
	u_int hops = 0;
	int new_bblock_links = 0, new_bblocks = 0, tot_new_bblocks =
		0, tot_bblocks = 0;
	...
	if (restricted_mode &&
		...
		igw_pack =
			igw_build_bentry(gnode_level, &igw_pack_sz, &new_bblocks);
 ...
 */
char *
tracer_pack_pkt(brdcast_hdr * bcast_hdr, tracer_hdr * trcr_hdr,
				tracer_chunk * tracer, char *bblocks, size_t bblocks_sz,
				int new_bblocks)
{
	bnode_hdr *bhdr;
	bnode_chunk *bchunk;
 ...
 */
int
tracer_unpack_pkt(PACKET rpkt, brdcast_hdr ** new_bcast_hdr,
				  ...
				  quadro_group * new_rip_quadg, int *real_from_rpos)
{
	...
	*real_from_rpos = 0;
	...
	if (tracer_verify_pkt
		(tracer, trcr_hdr->hops, rpkt.from, &rip_quadg, level))
		return -1;
 ...
 */
u_short
tracer_split_bblock(void *bnode_block_start, size_t bblock_sz,
					bnode_hdr *** bbl_hdr, bnode_chunk **** bbl,
	...
	for (loop = 0; loop <= 1; loop++) {
		...
		for (e = 0, x = 0; e < bblock_sz;) {
			...
			if (bblock_hdr->links <= 0
				|| bblock_hdr->links >= MAXGROUPNODE) {
				e += BNODEBLOCK_SZ(bblock_hdr->bnode_levels, 0);
				continue;
			}

			bsz =
				BNODEBLOCK_SZ(bblock_hdr->bnode_levels, bblock_hdr->links);
			...
			if (loop) {
				...
				bblist[x] =
					xmalloc(sizeof(bnode_chunk *) * bblock_hdr->links);
 ...
 */
int
tracer_store_bblock(u_char level, tracer_hdr * trcr_hdr,
					tracer_chunk * tracer, void *bnode_block_start,
					size_t bblock_sz, u_short * bblocks_found,
					char **bblocks_found_block, size_t * bblock_found_sz)
{
	map_node *node;
	map_gnode *gnode;
	void *void_node;
	...
	*bblocks_found_block = found_block = xmalloc(found_block_sz);
	for (i = 0; i < bb; i++) {
	  ...
	  skip_bmap:
		 ...
		 * it in network order */
		bsz =
			BNODEBLOCK_SZ(bblist_hdr[i]->bnode_levels,
						  bblist_hdr[i]->links);
 ...
 */
int
tracer_check_node_collision(tracer_hdr * trcr, int hop, map_node * node,
 ...
 */
int
tracer_store_pkt(inet_prefix rip, quadro_group * rip_quadg, u_char level,
				 ...
				 size_t * bblock_found_sz)
{
	...
	if (!level) {
		...
		gfrom =
			gnode_from_pos(tracer[from_tpos].node, me.ext_map[_EL(level)]);
		from = &gfrom->g;
		root_node = &me.cur_quadg.gnode[_EL(level)]->g;
	}
	...
	if (!level) {
		 ...
		 * can skip it. */
		i = map_find_bnode_rnode(me.bnode_map[level - 1],
								 me.bmap_nodes[level - 1], from);
		if (i != -1)
			skip_rfrom = 1;
	}
	...
	if (from_tpos >= 0) {		/* Is there a rnode in the tracer ? */
		...
		tracer_update_gcount(trcr_hdr, tracer, from_tpos,
							 qspn_gnode_count, me.int_map, me.ext_map,
							 level);
 ...
 */
int
flood_pkt_send(int (*is_node_excluded) (TRACER_PKT_EXCLUDE_VARS),
			   u_char level, int sub_id, int from_rpos, PACKET pkt)
{
	ext_rnode *e_rnode;
	...
	for (i = 0; i < me.cur_node->links; i++) {
		...
		if (!dst_node)
			continue;
		if (is_node_excluded
			(e_rnode, dst_node, from_rpos, i, level, sub_id))
			continue;
 ...
 */
int
exclude_glevel(TRACER_PKT_EXCLUDE_VARS)
{
...
int
exclude_from(TRACER_PKT_EXCLUDE_VARS)
{
	if (pos == from_rpos)
		return 1;
	return 0;
...
int
exclude_all_but_notfrom(TRACER_PKT_EXCLUDE_VARS)
{
...
int
exclude_from_and_glevel(TRACER_PKT_EXCLUDE_VARS)
{
...
tracer_pkt_recv(PACKET rpkt)
{
	...
	if (!level || level == 1) {
		...
		root_node = &me.cur_quadg.gnode[_EL(level)]->g;
		gfrom =
			gnode_from_pos(tracer[hops - 1].node, me.ext_map[_EL(level)]);
		from = &gfrom->g;
		tracer_starter =
	...
	if ((!level || ((me.cur_node->flags & MAP_BNODE) &&
		...
		tracer_pkt_build(rpkt.hdr.op, rpkt.hdr.id, bcast_hdr->sub_id,	/*IDs */
						 gid, level, bcast_hdr, trcr_hdr, tracer,	/*Received tracer_pkt */
