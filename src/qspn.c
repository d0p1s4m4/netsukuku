...
void
qspn_set_map_vars(u_char level, map_node ** map, map_node ** root_node,
 ...
 */
void
qspn_time_reset(int start_level, int end_level, int levels)
{
...
void
qspn_reset_counters(u_char levels)
{
 ...
 */
int
qspn_b_add(struct qspn_buffer *qb, u_char replier, u_short flags)
{
	qb->replies++;
...
int
qspn_b_find_reply(struct qspn_buffer *qb, int sub_id)
{
	int i;
 ...
 */
int
qspn_b_del_dead_rnodes(struct qspn_buffer **qb, map_node * root_node)
{
 ...
 */
void
qspn_b_del_all_dead_rnodes(void)
{
 ...
 */
void
update_qspn_time(u_char level, u_int new_qspn_time)
{
	struct timeval cur_t, t;
	int ret;
	...
	if (ret < 0 && abs(ret) > QSPN_WAIT_ROUND_MS_LVL(level)) {
		...
		ret =
			ret -
			(QSPN_WAIT_ROUND_MS_LVL(level) *
			 (ret / QSPN_WAIT_ROUND_MS_LVL(level)));
 ...
 */
void
qspn_inc_gcount(u_int * gcount, int level, int inc)
{
	int i;
 ...
 */
void
qspn_dec_gcount(u_int * gcount, int level, int dec)
{
	int i;
 ...
 */
void
qspn_reset_gcount(u_int * gcount, int level, int value)
{
	int i;
	for (i = 0; i < level; i++)
		gcount[i] = value;
 ...
 */
void
qspn_backup_gcount(u_int * old_gcount, int *gcount)
{
 ...
 */
void
qspn_remove_deads(u_char level)
{
	...
	for (i = 0; i < MAXGROUPNODE; i++) {
		...
		if ((node->flags & QSPN_OLD)) {
			...
			if (level) {
				...
				bmaps_del_bnode_rnode(me.bnode_map, (int *) me.bmap_nodes,
									  l, node);
			}
 ...
 */
void
qspn_new_round(u_char level, int new_qspn_id, u_int new_qspn_time)
{
	int i;
...
int
exclude_from_and_opened_and_glevel(TRACER_PKT_EXCLUDE_VARS)
{
	map_node *rn;
...
int
exclude_from_and_glevel_and_closed(TRACER_PKT_EXCLUDE_VARS)
{
...
int
exclude_from_and_glevel_and_notstarter(TRACER_PKT_EXCLUDE_VARS)
{
	...
	if ((!level || (node->flags & MAP_BNODE))
		&& !(node->flags & QSPN_STARTER))
		return 1;
...
qspn_send(u_char level)
{
	...
	ret_err = tracer_pkt_build(QSPN_CLOSE, me.cur_qspn_id[level], root_node_pos,	/*IDs */
							   upper_gid, level, 0, 0, 0,	/*Received tracer_pkt */
							   0, 0, 0,	/*bnode_block */
 ...
 */
int
qspn_open_start(int from_rpos, PACKET pkt_to_all, int qspn_id,
 ...
 */
int
qspn_unpack_pkt(PACKET rpkt, brdcast_hdr ** new_bcast_hdr,
				...
				int *new_root_node_pos, u_char * new_level,
				u_char * new_blevel, char *new_just_forward_it,
...
qspn_close(PACKET rpkt)
{
	...
	const char *ntop;
	char *old_bblock = 0;
	char do_real_qspn_action = 0, just_forward_it = 0, int_qspn_starter =
		0;
	...
	tracer_store_pkt(rpkt.from, &rip_quadg, level, trcr_hdr, tracer,
					 (void *) bhdr, bblock_sz, &old_bblocks_found,
	...
	if (!level
		|| me.bmap_nodes_closed[blevel] >= (me.bmap_nodes[blevel] - 1))
	...
	if (do_real_qspn_action && !just_forward_it) {
		...
		if (!just_forward_it && !not_closed &&
			...
			!(root_node->flags & QSPN_STARTER) && all_bnodes_are_closed) {
			rq = QSPN_OPEN;
...
qspn_open(PACKET rpkt)
{
	...
	if (!level
		|| me.bmap_nodes_opened[blevel] >= (me.bmap_nodes[blevel] - 1))
